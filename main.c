...
float firstNumber=0,secondNumber=0,thirdNumber=0,fourthNumber=0,fifthNumber=0,sixthNumber=0,answer=0; //variables for six operands and the answer.
...
char returnNumber(char a, char b, char c){
		//takes a reading from the keypad in a, 
		//concatenates the reading to b and copies it to c
...
float performCalculation(char input) {	
    ...
    if (flagmult==1) { //The flags raised by multiplication and dot keys on the keypad... 				
			...
			if(input=='3'){  //If you have pressed *, then ., then 3, perform square root operation
				...
				firstNumber=atof(numonefinal); //convert the string to float
				...
				firstNumber=answer;sprintf(numonefinal,"%0.2f",answer); //converts number into string
    ...
    if ((input == 'o' | input=='=')) { //If '=' is pressed
			if(pos[0]!=0 && pos[1]==0){  //if there is only one operator
				...
				firstNumber=atof(numonefinal); secondNumber=atof(numtwofinal); //convert addequate numbers
				...
				if(pos[0]==1){answer=firstNumber+secondNumber;} if(pos[0]==2){answer=firstNumber-secondNumber;}	if(pos[0]==3){answer=firstNumber*secondNumber;} if(pos[0]==4){answer=firstNumber/secondNumber;}				
				...
				secondNumber=0;firstNumber=answer;counter=0;pos[0]=0;pos[1]=0;		
			}	
			
			if(pos[1]!=0 && pos[2]==0){ //same for the rest
				
				firstNumber=atof(numonefinal); secondNumber=atof(numtwofinal);thirdNumber=atof(numthreefinal);
		 ...
		 if(pos[0]==1){y=firstNumber+secondNumber;} if(pos[0]==2){y=firstNumber-secondNumber;}	if(pos[0]==3){y=firstNumber*secondNumber;} if(pos[0]==4){y=firstNumber/secondNumber;}	
		 if(pos[1]==1){answer=y+thirdNumber;} if(pos[1]==2){answer=y-thirdNumber;}	if(pos[1]==3){answer=y*thirdNumber;} if(pos[1]==4){answer=y/thirdNumber;}
			 ...
			 secondNumber=0;thirdNumber=0;firstNumber=answer;counter=0;pos[0]=0;pos[1]=0;	
			}
			if(pos[2]!=0 && pos[3]==0){
				
				firstNumber=atof(numonefinal); secondNumber=atof(numtwofinal);thirdNumber=atof(numthreefinal);fourthNumber=atof(numfourfinal);
		 ...
		 if(pos[1]==1){x=y+thirdNumber;} if(pos[1]==2){x=y-thirdNumber;}	if(pos[1]==3){x=y*thirdNumber;} if(pos[1]==4){x=y/thirdNumber;}
			 if(pos[2]==1){answer=x+fourthNumber;} if(pos[2]==2){answer=x-fourthNumber;}	if(pos[2]==3){answer=x*fourthNumber;} if(pos[2]==4){answer=x/fourthNumber;}
				 ...
				 secondNumber=0;thirdNumber=0;fourthNumber=0;firstNumber=answer;counter=0;pos[0]=0;pos[1]=0;pos[2]=0;	
			}			
			if(pos[3]!=0 && pos[4]==0){
				
				firstNumber=atof(numonefinal); secondNumber=atof(numtwofinal);thirdNumber=atof(numthreefinal);fourthNumber=atof(numfourfinal);fifthNumber=atof(numfivefinal);
		 ...
		 if(pos[3]==1){answer=q+fifthNumber;} if(pos[3]==2){answer=q-fifthNumber;}	if(pos[3]==3){answer=q*fifthNumber;} if(pos[3]==4){answer=q/fifthNumber;}	 
			 ...
			 secondNumber=0;thirdNumber=0;fifthNumber=0;fourthNumber=0;firstNumber=answer;counter=0;pos[0]=0;pos[1]=0;pos[2]=0;pos[3]=0;
			}	
			if(pos[4]!=0){
		 ...
		 if(pos[4]==1){answer=w+sixthNumber;} if(pos[4]==2){answer=w-sixthNumber;}	if(pos[4]==3){answer=w*sixthNumber;} if(pos[4]==4){answer=w/sixthNumber;}			 
			 ...
			 secondNumber=0;thirdNumber=0;fifthNumber=0;sixthNumber=0;fourthNumber=0;firstNumber=answer;counter=0;pos[0]=0;pos[1]=0;pos[2]=0;pos[3]=0;
			}				
...
int password(char input) { 
		//concatenates the inputs of readKeypad() into a string,
		//which is converted to int before it's returned
    if(input == '0') {strcat(passEntry,"0");strcpy(passQuery,passEntry);} 
    if(input == '1') {strcat(passEntry,"1");strcpy(passQuery,passEntry);} 
    if(input == '2') {strcat(passEntry,"2");strcpy(passQuery,passEntry);} 
    if(input == '3') {strcat(passEntry,"3");strcpy(passQuery,passEntry);} 
    if(input == '4') {strcat(passEntry,"4");strcpy(passQuery,passEntry);} 
    ...
    if(input == '9') {strcat(passEntry,"9");strcpy(passQuery,passEntry);}
		...
		//a '1' is concatenated to passEntry, passEntry is then copied to passQuery
		//For example, we put a '2' in passEntry, then passQuery = passEntry
		//Then we concatenate a '3' to passEntry, which was '2', now it's '23'. passQuery = passEntry = 23
		//Concatenate '4' to passEntry, such that passEntry = '234', then passQuery=passEntry
